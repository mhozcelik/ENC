...
def ENC(X, y, Dist_type, X0_Center, X0_Var, X1_Center, X1_Var, 
         Importance, pred_train_original, train_mode, cut_off, Imp_type):
...
def ENC_Tuning(X_train, y_train, My_results, pred_train_original, Dist_type, Imp_type):
    ...
    pred, pred_class, exp_results = ENC(X_train,y_train, Dist_type,  \
                                        X0_Center, X0_Var, X1_Center, X1_Var, \
                                        Importance, pred_train_original, True, 0, Imp_type)
...
class ENCClassifier(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        ENC_Tuning(self.X_train, self.y_train,                         \
                self.My_results, self.pred_train_original,             \
                self.Dist_type, self.Imp_type)
...
# --------------
    ...
    def __predictions(self, X):
        temp_res_test0 = ENC(X, y = np.zeros(X.shape[0]), 
